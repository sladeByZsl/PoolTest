...
namespace QFramework
{
    ...
    {
        public static void Write(string name, string scriptsFolder, string scriptNamespace, ICodeInfo panelCodeInfo)
        {
            ...
            if (ViewControllerKit.Setting.IsDefaultNamespace)
            {
            ...
            writer.WriteLine(
                $"namespace {(string.IsNullOrWhiteSpace(scriptNamespace) ? ViewControllerKit.Setting.Namespace : scriptNamespace)}");
            writer.WriteLine("{");
            writer.WriteLine($"\tpublic partial class {name}");
            writer.WriteLine("\t{");

            foreach (var bindData in panelCodeInfo.BindInfos)
            {
                writer.WriteLine($"\t\tpublic {bindData.BindScript.Name} {bindData.Name};");
            }
    ...
    public class ViewControllerTemplate
    {
        public static void Write(string name, string scriptsFolder, string scriptNamespace)
        {
            ...
            writer.WriteLine(
                $"namespace {((string.IsNullOrWhiteSpace(scriptNamespace)) ? ViewControllerKit.Setting.Namespace : scriptNamespace)}");
            writer.WriteLine("{");
            writer.WriteLine($"\tpublic partial class {name} : ViewController");
            writer.WriteLine("\t{");
